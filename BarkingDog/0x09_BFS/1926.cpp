 ...
 * 이어지지 않은 탐색범위들이 존재한다. (그림)
 * 이 범위들의 시작점을 이중포문으로 찾아내서
 * 기존에 작성했던 BFS 코드에 넣고 돌리면 됨
 * 탐색중인 범위(그림)의 크기는 큐에서 원소를 뺄 때 마다 하나씩 증가
 */
...
int main(void)
{
    ...
    int mx = 0;  // 그림의 최댓값
    int num = 0; // 그림의 수
    for (int i = 0; i < n; i++) // board를 이중포문으로 돌면서 이어지지 않은 그림들의 시작점을 찾는다.
    {
        for (int j = 0; j < m; j++)
